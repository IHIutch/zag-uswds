...
export class Accordion extends Component<accordion.Props, accordion.Api> {
  initMachine(context: accordion.Props): VanillaMachine<AccordionSchema> {
    return new VanillaMachine(accordion.machine, { ...context })
  }
  ...
  private renderItem(itemEl: HTMLElement) {
    ...
    if (trigger) spreadProps(trigger, this.api.getTriggerProps({ value }))
    if (content) spreadProps(content, this.api.getContentProps({ value }))
  }
}
...
export function accordionInit() {
  document
    ...
    .forEach((targetEl) => {
      const acc = new Accordion(targetEl, { id: targetEl.id || 'accordion' })
      acc.init()
    })
}
